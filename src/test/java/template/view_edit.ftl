...
<script type="text/javascript">
    ...
    var url = "<#noparse>${ctx}</#noparse>/${t.name}/update";
    var form = "form";
    ...
    function init() {
        var model = <#noparse>${model}</#noparse>;
        ex.form.fill(form, model);

        ex.form.onSubmitEvent("btnSubmit", function (data) {
            layui.layer.load(2);
            var newModel = data.field;
            ex.ajax.postJson(url, newModel, function (result) {
                layui.layer.closeAll('loading');
                if (result.code == 1) {
                    top.layer.msg("修改成功", {icon: 1});
                    ex.common.putTempData("form", {isOk: true, data: newModel});
                    // 关闭当前iframe弹出层
